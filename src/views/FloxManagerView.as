...
public class FloxManagerView extends MovieClip
{
	...
	public function init():void
	{
		...
		managerUI.createKeybtn.addEventListener(MouseEvent.CLICK,doCreateKey);
		...
		managerUI.attr.addEventListener(Event.CHANGE,onPlayerAttributeChanged);
	...
	private function doCreateKey(e:MouseEvent):void{

		var key:String=flox.getUID();
		managerUI.reviewkey.text=key;
		DebugTrace.msg("FloxManagerView.doCreateKey key="+key);
		flox.signupWithKey(key);
	}
...
//		}
	...
	private function doUpdatePlayer(e:MouseEvent):void{
		var value:String=String(managerUI.new_value.text);
		var result:*;
		switch(playerAttr){
			case "assets":
			case "items":
			case "rewards":
			case "commander_items":
			case "chargeslog":
			case "saved":
		...
		DataContainer.EDITED_VALUE={attr:playerAttr,value:result};
		flox.playerEditor(ownerId);


		showResult("FloxManagerView.doUpdatePlayer playerAttr="+playerAttr+", result="+JSON.stringify(result));
	}
	private function onPlayerAttributeChanged(e:Event):void{
