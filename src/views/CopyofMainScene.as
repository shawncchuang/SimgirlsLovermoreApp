package views
{
	...
	public class MainScene extends Scenes
	{
		...
		private function initScene():void
		{
			...
			_scroll.boundWidth = 1024;  
			_scroll.boundHeight = 768; 
			_scroll.content = container;  
			// accepted values: Orientation.AUTO, Orientation.VERTICAL, Orientation.HORIZONTAL
			_scroll.orientation = Orientation.AUTO; 
		...
		private function onSceneTouched(e:TouchEvent):void
		{
			...
			var touch:Touch=e.getTouch(scene);
			hover = e.getTouch(target, TouchPhase.HOVER);
			...
			/*try
			{
				var pos:Point = touch.getLocation(scene);
		...
		private function doSceneEnterFrame(e:Event):void
		{
			...
			var movieX:Number=Math.floor((container.width-stageW)/(Math.abs(new_gx-gx)));
			var movieY:Number=Math.floor((container.height-stageH)/(Math.abs(new_gy-gy)));
			if(gx<scroll_area)
			{
				if(hover)
				bgX+=movieX;
			...
			if(gx>stageW-scroll_area)
			{
				if(hover)
				bgX-=movieX;
				
				if(bgX<container.width-stageW)
				{
			...
			if(gy>stageH-scroll_area)
			{
				if(hover)
				bgY-=movieY;
				
				if(bgY<container.height-stageH)
				{
					bgY=-(container.height-stageH);
				}
				
			}
			//DebugTrace.msg(String(gx)+" ; "+String(gy));
			if(gx!=0 && gy!=0)
			{
				
				var bgTween:Tween=new Tween(container,0.5);
				bgTween.animate("x",bgX);
				bgTween.animate("y",bgY);
