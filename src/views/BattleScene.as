...
public class BattleScene extends Sprite
{
    ...
    /*
     ...
     var loaderQueue:LoaderMax=ViewsContainer.loaderQueue["SkillTest"];
    ...
    {
        var loaderQueue:LoaderMax=ViewsContainer.loaderQueue["background"];
        var swfloader:SWFLoader = loaderQueue.getLoader("background");
    ...
    private function onClickCardNote(e:MouseEvent):void
    {
        ...
        {
            var loaderQueue:LoaderMax=ViewsContainer.loaderQueue["cardnote"];
            var swfloader:SWFLoader = loaderQueue.getLoader("cardnote");
    ...
    private function removeCardNote():void
    {
        ...
        {
            var loaderQueue:LoaderMax=ViewsContainer.loaderQueue["cardnote"];
            loaderQueue.getLoader("cardnote").unload();
    ...
    private function onBattleAlertFadeOut(e:TimerEvent):void
    {
        ...
        try
        {
            var loaderQueue:LoaderMax=ViewsContainer.loaderQueue["attention"];
            loaderQueue.getLoader("attention").unload();
        }
        catch(e:Error)
        {
    ...
    private function showupSPAni(act:String):void
    {
        ...
        {
            var loaderQueue:LoaderMax=ViewsContainer.loaderQueue["spbg"];
            var swfloader:SWFLoader = loaderQueue.getLoader("spbg");
                ...
                else
                {
                    ...
                    switch(attack_member.power.type)
                    {
                        ...
                        default:
                            ...
                            TweenMax.delayedCall(0.5,onAttack);
                    ...
                    /*
                     ...
                     //TweenMax.to(attack_member,0.2,{x:Math.floor(movingX),y:Math.floor(movingY),onComplete:onHopComplete});
                     }

                     }
                     //if
                     if(e.target.currentFrame== e.target.totalFrames){
                     e.target.removeEventListener(Event.ENTER_FRAME,doHopping);
                     e.target.stop();
                     TweenMax.to(attack_member,0.2,{x:Math.floor(movingX),y:Math.floor(movingY),onComplete:onHopComplete});
                     }
                    ...
                    function onHopComplete():void{
                        ...
                        //TweenMax.killTweensOf(onHopComplete);
                        TweenMax.delayedCall(1,onAttack);
                    }
    ...
    private function doAttackHandle():void
    {
        ...
        if(member.power.label.indexOf("_")!=-1 || boss.indexOf(member.power.name)==-1)
        {
            //attack skill
            ...
            if(loaderQueue){
                ...
                var skillAni:MovieClip=swfloader.getSWFChild(gender) as MovieClip;
                bodyMC=skillAni.body;
            }else{
                var actMC:MovieClip=member.skillAni.body.act;
                bodyMC=member.skillAni.body;
            }
        ...
        else
        {
            ...
            actMC=member.character.body.act;
            bodyMC=member.character.body;
            bodyMC.addEventListener(Event.ENTER_FRAME,doActPlaying);
        }
        //if
    }
    ...
    private function doActPlaying(e:Event):void
    {
        ...
        var currentFrame:Number= e.target.currentFrame;
        ...
        if(currentFrame==totalFrames){
            ...
            onActPlayComplete(e.target as MovieClip);
        }
...
//				//if
        ...
        /*
         var effect:String=attack_member.power.effect;
         if(effect!="regenerate" && effect!="heal" && effect!="mind_ctrl" && effect!="shield")
         {
         ...
         */
        var hitframes:Array=[Math.floor(totalFrames)];
        if(hitframes.length>0 && currentFrame==hitframes[hitT])
        {
            hitHandle();
        }
    ...
    private function onActPlayComplete(target:MovieClip):void{
        ...
        TweenMax.delayedCall(0.5,onBeforeAttackComplete);

        function onBeforeAttackComplete():void
        {
